...
public class ContactDeletionTests extends TestBase {
  ...
  public void ensurePreconditions() {
    if (app.contact().all().size() == 0) {//если нет контакта который можно редактировать -> создаем его
      app.contact().create(new ContactData().withFirstname("aa").withLastname("aa").withAddress("test3").withPhoneH("test4").withPhoneM("test5").withPhoneW("test6").withEmail("test7").withGroup("test4"), true);
    }
